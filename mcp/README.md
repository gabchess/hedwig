...
```
`consult` takes `{ "request": <payment request> }` and returns the same response `@hedwig/consult` returns: `proceed`, `band`, `verdict`, `support`, `results`, `floorIds`, `advisory: true`. `support` shows how much of the owner's checklist was proven and how strong the proof was; callers act on `proceed`.
